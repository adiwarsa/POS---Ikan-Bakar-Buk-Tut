<form action="{{ $food->id ? route('food.update', $food->id) : route('food.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <input type="hidden" name="type" value="minuman">
        </div>
    <div class="mb-3">
		<x-label for="jenis" :value="__('jenis')" />
								<select class="form-select" id="jenis" name="jenis">
										<option
											value="Soft Drink"
											@selected(old('jenis') == 'Soft Drink')>Soft Drink</option>
										<option
											value="Hot"
											@selected(old('jenis') == 'Hot')>Hot</option>
                                        <option
											value="Juice"
											@selected(old('jenis') == 'Juice')>Juice</option>
                                        <option
											value="Beer"
											@selected(old('jenis') == 'Beer')>Beer</option>
                                        <option
											value="Wine"
											@selected(old('jenis') == 'Wine')>Wine</option>
								</select>
	</div>
